...
layout(binding = 4) uniform sampler2D s_Projection;  // The image to project if this light is a projector
...
vec4 GetWorldPos(vec2 uv) {
	...
	float zOverW = texture(s_CameraDepth, uv).r * 2 - 1; // Convert depth from normal to clip space
