...
void SceneBuilder::Initialize()
{
	...
	mat->Set("a_AmbientPower", 0.2f); 
	mat->Set("a_MatShininess", 256.0f); 
	mat->Set("a_Lights[0].Pos", { 2, 0, 0 }); 
	mat->Set("a_Lights[0].Color", glm::vec3(1.0f, 0.0f, 1.0f) * 0.3f);  
	mat->Set("a_Lights[0].Attenuation", 1.0f / 10.0f);
	mat->Set("a_Lights[1].Pos", { -2, 0, 0 });
	mat->Set("a_Lights[1].Color", glm::vec3(0.0f, 1.0f, 0.0f) * 0.3f);
	mat->Set("a_Lights[1].Attenuation", 1.0f / 10.0f);
	...
	// Creates our main camera
	{
		...
		// TODO: Note that we are adding a second attachment, this one to color slot 1
		static_assert(false);
	...
	for (int ix = 0; ix < numMonkeys; ix++) {
		...
		light.Color = glm::vec3(glm::sin(ix * step) + 1.0f, glm::cos(ix * step) + 1.0f, glm::sin((ix * step) + glm::pi<float>()) + 1.0f) / 2.0f * 0.9f; 
